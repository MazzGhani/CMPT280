...
public class Ship {
    ...
    public boolean isOverloaded() {
        ...
        if(cargo.isFull()){
            throw new RuntimeException("Max capacity cargo is too full ");
        }
        else {
            return true;
        }
    ...
    public int sacksOfGrainType(Grain type) {
        ...
        LinkedList280<Sack> box[]= new LinkedList280[5];

        for(int i=0;i<5;i++){
                box[i]= new LinkedList280<Sack>();
        }

        for(int i=0; i< Grain.values().length;i++){
            switch(box[i].getClass().toString()){
                case "WHEAT":
                    box[0].insert(type.compareTo(Sack));
            }
        }
